...
import { createReplyComment, deleteReplyComment, modifyComment } from '../../api/postData';
import { getCommentListFromDB } from '../../features/comment/commentSlice';
import { CiMenuKebab } from 'react-icons/ci';
import { BsPencil, BsTrash } from 'react-icons/bs';

export default function ReplyCommentCard({selectedPostId, selectedCommentId, comment, comment : {_id, content, createdAt, writer : {name, profileImg}}}) {
  ...
  const onSubmitCreateReplyHandle = async (e) => {
    ...
    try {
      const response = await createReplyComment({
        ...
        replyTo : _id,
        rootCommentId: selectedCommentId,
      })
  ...
  const onSubmitReplyModifyHandle = async (e) => {
    e.preventDefault();
    try {
  ...
  return (
    <>
      <div className={userInfo ? 'replyComment flex min-h-[70px]' : 'replyComment flex min-h-[40px]'}>
        ...
        <div className='w-full flex flex-col'>
          {!toggleModifyComment && (
            <>
              ...
              <div className='mt-0.5 mobile:text-xs tablet:text-sm lg:text-base xl:text-lg whitespace-pre-wrap'>
                {content.match(/^(@\w+)\s(.*)$/) ? (
                  <>
                    <span className='bg-slate-500 rounded-md font-bold mr-1'>&nbsp;{content.match(/^(@\w+)\s(.*)$/)[1]}&nbsp;</span>
                    {content.match(/^(@\w+)\s(.*)$/)[2]}
                  </>
                ) : content}
              </div>
              ...
              {replyForm && (
                <div className=' flex w-full mt-3 mobile:h-[60px] lg:h-[85px]'>
                  ...
                  <div className='relative w-full overflow-x-hidden scrollbar-thin scrollbar-thumb-rounded-md  scrollbar-track-white dark:scrollbar-track-slate-500 scrollbar-thumb-slate-300 dark:scrollbar-thumb-slate-700 '>
                    <form className='mobile:w-[140px] tablet:w-[340px] lg:w-[575px] xl:w-[655px]'>
                      <textarea ref={textareaRef} rows={1}  placeholder={`@${name}`} value={replyText} onChange={onChangeHandle} onFocus={() => setFocus(true)} className={focus ? 
          ...
          {toggleModifyComment && (
            <div className='w-full overflow-x-hidden '>
              <form className='mobile:min-h-[64px] tablet:min-h-[72px] lg:min-h-[76px] xl:min-h-[80px]'>
                ...
                <div className='flex justify-end mt-2 mobile:mt-1 mobile:text-xs mobile:h-[25px] lg:h-[35px]'>
                  ...
                  <button type='submit'  disabled={replyText === ''} onClick={onSubmitReplyModifyHandle} className={
                    replyText !== '' ? 'rounded-md bg-green-500 mobile:w-[25px] mobile:h-[25px] lg:w-[50px] lg:h-[35px]'
                    : 'rounded-md bg-slate-500 dark:bg-slate-900 cursor-not-allowed mobile:w-[25px] mobile:h-[25px] lg:w-[50px] lg:h-[35px]'}>
                    수정
                  </button>
