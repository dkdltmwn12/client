...
import ModalArticleDetails from '../../../portal/articleDetails/ModalArticleDetails';
import { useDispatch, useSelector } from 'react-redux';
import { getCommentListFromDB } from '../../../features/comment/commentSlice';

export default function ArticleCard({article,  article : {_id, myPositionType, title, name, contents, createdAt, microType, gameStyleType, tier, tier : {tierName, rank, recentChampList}}}) {
  ...
  return (
    <>
      <article onClick={() => setToggleDetails(true)}
        className='w-full h-[300px] text-xs flex flex-col bg-slate-300 dark:bg-slate-700 rounded-md mobile:w-[295px] mobile:p-3 mobile:m-1 tablet:w-[255px] tablet:p-2 lg:w-[270px] xl:w-[295px] xl:p-3 xl:m-1.5 cursor-pointer'>
        <div className='flex max-h-[52px] '>
          ...
          <div className=''>
            ...
            <div className='flex text-xs mt-1 mobile:leading-[13px] tablet:leaing-[15px]'>
              <img className=' w-[20px] h-[20px] mobile:w-[13px] mobile:h-[13px] tablet:w-[15px] tablet:h-[15px]' src={title.includes('솔로랭크') ? tier !== 'unranked' ? `./img/emblem/emblem-${tierName}.png` : `./img/emblem/emblem-${tier}.png` : title.includes('자유랭크') ? tier !== 'unranked' ? `./img/emblem/emblem-${tierName}.png` : `./img/emblem/emblem-${tier}.png` : './img/emblem/emblem-unranked.png'}  alt={tier !== 'unranked' ? tierName : 'unranked'}/>
              {title.includes('솔로랭크') ? tierName !== 'unranked' ? (<span className='ml-2 '>{shortenTierName(tierName)}{parseTierNum(rank)}</span>) : (<span className='ml-1'>Unranked</span>)
                : title.includes('자유랭크') ? tierName !== 'unranked' ? (<span className='ml-2'>{shortenTierName(tierName)}{parseTierNum(rank)}</span>) : (<span className='ml-1'>Unranked</span>)
      ...
      {toggleDetails && ( 
        <Modal>
          <ModalArticleDetails article={article} setToggleDetails={setToggleDetails} selectedPostCommentList={selectedPostCommentList}/>
        </Modal>
      )}
    </>
  )
